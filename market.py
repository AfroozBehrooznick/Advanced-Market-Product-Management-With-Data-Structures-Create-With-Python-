...
#Trun a list to a bst tree in order log(n)
...
def inorderBST(node):
    result = []
    if node:
        result.extend(inorderBST(node.left))   
        result.append(node.value)
        result.extend(inorderBST(node.right))  
    return result
...
def CalculateMedian(linked_list):
...
class ProductBST:
    ...
    #find minimum vlaue in a part of tree
...
class PriceBST:
    ...
    def _delete(self, node, price):
        ...
        if price < node.product.priceOfThisYear:
            ...
            #Smallest node in right tree for replacement 
            temp = self.min_value_node(node.right)
            node.product = temp.product
    ...
    def findProductByPrice(self, price):
    ...
    def rangeOfPrice(self, range1, range2):
        if range2 < range1:
            range2, range1 = range1, range2

        productRange1 = self.findProductByPrice(range1)
        productRange2 = self.findProductByPrice(range2)
...
class Product:
    def __init__(self, productName, productQcode, listOfPrice):
        ...
        self.medianPrice = CalculateMedian(self.listOfPrice)
############
...
#button 1 add new product to our products
def Add_new_product():
...
#Add new price of other years
def AddNewPrice(name):
    ...
    while True:
        ...
        if added:
            ...
            product.medianPrice = CalculateMedian(product.listOfPrice)
            break
        else:
...
while flagForWholeProgramme:
    ...
    if inputNumber == 1:
        if int(input("Do you want to add a product or new price of a product:  1=product , 0=New price  :")):
            ...
            AddNewPrice(nameofproduct)

    #Delete one of products
    elif inputNumber == 2:
        name = input("Enter the name of the product to delete: ")
        DeleteProduct(name)

    #Get median of list of price
    elif inputNumber == 3:
        for product in product_manager.products:
            print(f"Product Name: {product.productName}, Median Price: {product.medianPrice}")

    #Get a list of products bise on sorted smallest price
    elif inputNumber == 4:
        product_manager.price_bst.sortedSmallestPrice()


    #Get a list of products bise on sorted biggest price
    elif inputNumber == 5:
        product_manager.price_bst.sortedBiggestPrice()


    #Get a list of products bise on a range of price 
    elif inputNumber == 6:
        range1 = int(input("Enter the lower range: "))
        range2 = int(input("Enter the upper range: "))
        product_manager.price_bst.rangeOfPrice(range1, range2)

    #Exit the whole programe
    elif inputNumber == 7:
        break

    #Check if your input is not right    
    else:
